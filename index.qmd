...
# This outputs a list with distint cell combinations and counts of taxa in each
test <- 
  czekanowski_data_prep(x = pbdb_sensitivity |> filter(early_interval == "Induan" & ex==0 & fad >=252.17),
                 cell = "cell",accepted_name = "accepted_name")

# Next add the minimum field to each
ubb <- purrr::map(test, ~.x |> mutate(minimum = pmin(count_cell_x, count_cell_y)))
# Next compute the Czekanowski index
iii <- lapply(ubb, czekanowski_similarity)
