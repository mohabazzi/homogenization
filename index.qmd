...
```{r}
#' @ex=1 in the pbdb are the victims.
#' @ex=0 in the pbdb are the survivors.
...
# Determine cell frequency over survival/victim data.
pbdb_sensitivity |> 
  group_by(ex,early_interval) |> 
  summarize(unique_cells = n_distinct(cell)) # Changhsingian victims are not needed.
...
sV <- pbdb_sensitivity |>
  filter(early_interval == "Induan" & ex==1 & fad >= 252.17) |>
  group_split(cell_id) |> lapply(as.data.frame)
...
sI_boot <- purrr::map(1:999, ~ { sI |> sample(13,replace = F) })
sV_boot <- purrr::map(1:999, ~ { sV |> sample(13,replace = F) })
sC_boot <- purrr::map(1:999, ~ { sC |> sample(13,replace = F) })

# Next step is to bind them.
sI_boot_combined <- list()

for(i in seq_along(sI_boot)) {
  sic <- purrr::map_dfr(sI_boot[[i]], bind_rows)
  sI_boot_combined[[i]] <- sic
}

sV_boot_combined <- list()

#
for(i in seq_along(sV_boot)) {
  svC <- purrr::map_dfr(sV_boot[[i]], bind_rows)
  sV_boot_combined[[i]] <- svC
}

#
sC_boot_combined <- list()

for(i in seq_along(sC_boot)) {
  scc <- purrr::map_dfr(sC_boot[[i]], bind_rows)
  sC_boot_combined[[i]] <- scc
}
