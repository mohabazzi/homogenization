...
The occurrence of a given taxa between distinct cells are evaluated against each other. We first define a dataframe that list all the unique cells and unique taxon name for each stage-level dataframe. For the 
...
# Induan.
ind_combs <- gridComb(x = induan_pbdb,cell = cell,accepted_name = accepted_name) # 20*93 = 1860 rows.
...
cz_ind_prep <- czekanowski_splits(ind_joined) # May or may have not worked. Return back to this step.
...
sumRes_03 |> 
  ...
  labs(x = "Great Circle Distance", 
       ...
       subtitle = "Bootstrap results from subsampling by cells and occurrences") +
  theme_minimal() +
