...
This section needs to be explained.
...
subsampling_fun <-
  function(x, n_boot = 999, sample_size = 13, seed = 5) {
  set.seed(seed)
  # Bootstrap samples.
...
# Induan victims.
induan_victims_jaccard <-   jaccard_similarity(sV_boot)
# Changhsingian survivals.
...
# Mean jaccard for the Induan victims.
aIvJ <- 
  bind_rows(induan_victims_jaccard) |>
  group_by(cell_x, cell_y) |> 
  summarise(avg_jaccard = mean(jaccard_similarity)) |> 
  rename("x.cell" = "cell_x", "y.cell" = "cell_y")
...
```{r}
# Induan results: survivors & victims.
mRes_01 <- induan_res_matrix[,c(1:7,10)] |> left_join(aIsJ,by = c("x.cell","y.cell"))
mRes_02 <- induan_res_matrix[,c(1:7,10)] |> left_join(aIvJ,by = c("x.cell","y.cell"))
# Changhsingian results: survivors.
...
sumRes_08 <-
  ...
  mutate(label = 'Induan victims',label = as.factor(label)) |> 
  mutate(cutdist = cutdist,
         cutdist = factor(cutdist,levels = c("0","2000","4000","6000","8000","10000","12000","14000","16000","18000","20000"))) |> 
  mutate(ci = se * qt(.975, n - 1), ci = as.numeric(ci)) |> 
  as.data.frame()
