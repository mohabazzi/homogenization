...
Jood Al Aswad<sup>1\*</sup>, Justin L. Penn<sup>2</sup>, Pedro Monarrez<sup>1</sup>, Mohamad Bazzi<sup>1</sup>, Curtis Deutsch<sup>2</sup>, Jonathan L. Payne<sup>1</sup><br>
...
The globe is divided into a grid of equal-area icosahedral hexagonal cells using the `hexagrid()` function in `iscosa`. In `hexagrid(deg = x)`, is roughly equivalent to longitudinal degrees, so that a degree of 1 is roughly equal to 111 km. This selects a tessellation vector, which translates to the amount of area you select for each cell. In our specified grid, each cell is roughly 600,000 km^2^ and results in a grid of 642 cells.
